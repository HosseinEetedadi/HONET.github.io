...
class Job:
    ...
    class TransmissionAction(Enum):
        ...
        UPSCHEDULER_TO_UPWORKER = 9
        UPWORKER_TO_UPSCHEDULER = 10
        SCHEDULER_TO_UPCLUSTER = 11
        UPCLUSTER_TO_UPSCHEDULER = 12
        UPCLUSTER_TO_SCHEDULER = 13
        UPSCHEDULER_TO_CLUSTER = 14
        UPSCHEDULER_TO_CLOUD = 15
        CLOUD_TO_UPSCHEDULER = 16
    ...
    def a_forwarded(self, to_node_uid=-1, to_cloud=False, to_cluster_id=-1, to_up=0):
        ...
        if self._forwarded_to_cluster_id != 3:
            if to_up == 0:
                if self._forwarded_to_cluster_id == -1:
                    # print(f"job={self._uid}  TO UP IS ZERO :)")
                    self._forwarded_to_cluster_id = to_cluster_id
            elif to_up == 1:
                # print(f"job={self._uid}   TO UP IS ONE :)")
                self._forwarded_to_cluster_id = 3
    ...
    def a_done(self):
        ...
        if (self._rejected or (self._executed and self._forwarded)) and not (
                len(self._transmission_actions_list) in [2, 3, 4, 5, 6, 7, 8]):
            raise RuntimeError(
    ...
    def compute_duration(job, job_duration_type, machine_speed: float):
        ...
        # print(f"This is the job duration = {job.get_job_duration()}")
    ...
    def get_transmission_next_action(self) -> TransmissionAction:
        # print(f"job = {self._uid}, self._transmission_next_action = {self._transmission_next_action}")
    ...
    def get_forwarded_to_cluster_id(self):
        # print(f"job = {self._uid}  This is the cluster id = {self._forwarded_to_cluster_id}")
    ...
    def get_forwarded_to_node_id(self):
        # print(f"self._forwarded_to_node_id = {self._forwarded_to_node_id}")
    ...
    def set_transmission_next_action(self, action: Job.TransmissionAction):
        # print(f"job = {self._uid}, this is the action: {action}")
        # print(f"And this is the transmission action list: {self._transmission_actions_list} ")
        ...
        # print(f"job = {self._uid}, Former self._transmission_next_action = {self._transmission_next_action}")
